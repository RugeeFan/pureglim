...
export const quoteRequestSchema = z
  .object({
    ...
    hourlyHours: z
      ...
      .min(HOURLY_MIN_HOURS, `Hourly bookings start from ${HOURLY_MIN_HOURS} hours.`)
      .max(HOURLY_MAX_HOURS, `Hourly bookings cap at ${HOURLY_MAX_HOURS} hours.`)
      .nullable()
      .optional(),
  ...
  .superRefine((data, context) => {
    ...
    if (data.serviceType === "hourly") {
      if (
        data.hourlyHours == null ||
        !Number.isInteger(data.hourlyHours) ||
        data.hourlyHours < HOURLY_MIN_HOURS ||
        data.hourlyHours > HOURLY_MAX_HOURS
      ) {
        context.addIssue({
          code: z.ZodIssueCode.custom,
          message: `Choose between ${HOURLY_MIN_HOURS} and ${HOURLY_MAX_HOURS} hours.`,
          path: ["hourlyHours"],
        });
      }
    }
