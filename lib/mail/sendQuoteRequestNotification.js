...
import { getBookingPricingDetails } from "../services/bookingMeta";
...
function buildPricingRows(request) {
  return getBookingPricingDetails(request).rows.map((row) => [
    row.label,
...
function buildText(request, intro, footer) {
  return [
    ...
    request.hourlyHours ? `Cleaner hours: ${request.hourlyHours} hrs` : null,
    ...
    ...buildPricingRows(request).map(([label, value]) => `${label}: ${value}`),
    request.referralCode ? `Referral code: ${request.referralCode}` : null,
    request.referrer?.fullName ? `Referred by: ${request.referrer.fullName}` : null,
