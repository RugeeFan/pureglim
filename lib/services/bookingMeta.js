...
export const REFERRAL_DISCOUNT_AMOUNT = 20;
export const REFERRAL_COMMISSION_AMOUNT = 10;
...
export function computeCommissionAmount(baseAmount) {
  ...
  return REFERRAL_COMMISSION_AMOUNT;
}

export function computeBookingAmounts(originalAmount, hasReferralDiscount) {
  ...
  const discountAmount = hasReferralDiscount ? REFERRAL_DISCOUNT_AMOUNT : 0;
  const finalAmount = Math.max(originalAmount - discountAmount, 0);
  const commissionBaseAmount = hasReferralDiscount ? finalAmount : null;
